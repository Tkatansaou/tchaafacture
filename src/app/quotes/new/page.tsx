...
const emptyLine = (): LineItem => ({ _key: uid(), description: '', quantity: 1, unitPrice: 0, total: 0 })
...
export default function NewQuotePage() {
  ...
  useEffect(() => {
    Promise.all([getCustomers(), getSettings()]).then(([c, s]) => {
      setCustomers(c)
      setExpiryDate(addDays(today(), s.paymentTerms || 30))
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold tracking-tight">Nouveau devis</h1>
          </div>
              ...
              <div className="space-y-2">
                {lines.map((line) => (
                  <div key={line._key} className="grid grid-cols-[1fr_80px_130px_130px_40px] gap-2 items-center">
                    <Input placeholder="Description" value={line.description} onChange={(e) => updateLine(line._key, 'description', e.target.value)} />
                    ...
                    <button onClick={() => lines.length > 1 && setLines(p => p.filter(l => l._key !== line._key))}
              ...
              {lines.map((line, idx) => (
                <div key={line._key} className="rounded-lg border p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => lines.length > 1 && setLines(p => p.filter(l => l._key !== line._key))} className="text-red-400">
                      <Trash2 className="h-4 w-4" />
                    </button>
                  </div>
                  <Input placeholder="Description" value={line.description} onChange={(e) => updateLine(line._key, 'description', e.target.value)} />
                  <div className="grid grid-cols-2 gap-2">
                    <div><label className="mb-1 block text-xs text-muted-foreground">Qté</label>
                      <Input type="number" value={line.quantity || ''} onChange={(e) => updateLine(line._key, 'quantity', e.target.value)} /></div>
                    <div><label className="mb-1 block text-xs text-muted-foreground">Prix HT</label>
                      <Input type="number" value={line.unitPrice || ''} onChange={(e) => updateLine(line._key, 'unitPrice', e.target.value)} /></div>
                  </div>
                </div>
              ))}
            </div>

            <Button variant="outline" onClick={() => setLines(p => [...p, emptyLine()])} className="mt-4 w-full" size="sm">
            ...
            <div className="mt-6 ml-auto w-full max-w-xs space-y-2">
              ...
              <div className="flex items-center justify-between gap-3 text-sm">
                ...
                <div className="flex items-center gap-2">
                  <select value={taxKey} onChange={(e) => handleTaxChange(e.target.value as TaxKey)}
                    ...
                    {TAX_OPTIONS.map(o => <option key={o.key} value={o.key}>{o.label}{o.rate !== null ? ` (${o.rate}%)` : ''}</option>)}
                  </select>
                  {taxKey === 'custom' && (
        ...
        <Card>
          ...
          <CardContent>
            <Textarea placeholder="Conditions, remarques..." value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} />
          </CardContent>
        </Card>

        <div className="flex justify-end gap-3 pb-6">
          <Button variant="outline" asChild><Link href="/quotes">Annuler</Link></Button>
          <Button variant="outline" onClick={() => handleSubmit('draft')} disabled={saving}><Save className="mr-2 h-4 w-4" />Brouillon</Button>
          <Button onClick={() => handleSubmit('sent')} disabled={saving} className="rounded-full px-6"><Send className="mr-2 h-4 w-4" />Enregistrer</Button>
        </div>
      </div>
    </DashboardLayout>
  )
